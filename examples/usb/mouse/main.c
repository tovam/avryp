...
#define PORTLED PORT$avryp_port_led
#define DDRLED  DDR$avryp_port_led
#define PINLED  $avryp_pin_led

void waitms(int x){
    for(int j = 0; j<x/2; j++) { // wait X ms
...
int main() {
	...
	i = 0;
	unsigned int N = 10000/$avryp_speed;
    while(1) {
        wdt_reset(); // keep the watchdog happy
        usbPoll();
		if(i % N==0){
			TOGGLE(PORTLED, PINLED);
			i=0;
		}
